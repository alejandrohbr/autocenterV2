...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, VehicleDiagnosticComponent, DiagnosticDisplayComponent, CustomerSearchComponent, AuthorizationRequestComponent, InvoiceUploadComponent, XmlUploadComponent, ProductClassificationComponent, LostSalesReportComponent, BudgetPreviewComponent, PreOcValidationComponent, FooterComponent],
...
export class DashboardComponent implements OnInit {
  ...
  isLoadingPreOCOrders: boolean = false;
  ...
  async setActiveView(view: 'dashboard' | 'new-order' | 'customer-search' | 'admin-validation' | 'pre-oc-validation' | 'not-found-products') {
    this.activeView = view;

    // Cargar datos específicos según la vista
    if (view === 'pre-oc-validation') {
      await this.loadPendingPreOCOrders();
    }
  }
  ...
  async onAuthorizationSubmitted(items: any[]) {
    ...
    try {
      ...
      const message = `Autorización guardada exitosamente.\n\n` +
        `✓ ${authorizedItems.length} servicios autorizados (Total: $${formatCurrencyValue(totalAutorizado)})\n` +
        `✗ ${rejectedItems.length} servicios NO autorizados (Total: $${formatCurrencyValue(totalRechazado)})\n\n` +
      ...
      // Recargar el pedido desde la BD para obtener datos actualizados
      ...
      // Reabrir el detalle del pedido si se desea
      if (orderId) {
        const updatedOrder = this.orders.find(o => o.id === orderId);
        if (updatedOrder) {
          await this.viewOrderDetail(updatedOrder);
        }
      }
    } catch (error: any) {
  ...
  async loadPendingPreOCOrders() {
    ...
    if (this.isLoadingPreOCOrders) {
      console.log('Ya hay una carga de órdenes Pre-OC en progreso, ignorando...');
      return;
    }

    this.isLoadingPreOCOrders = true;
    ...
    try {
      ...
      const mappedOrders = await Promise.all((data || []).map(async (order: any) => {
        let processedProductsCount = 0;
        let productosPorProveedor: any[] = [];
        ...
        if (order.id) {
          ...
          // Cargar productos XML para cálculos financieros
          try {
            ...
            productosPorProveedor = this.xmlProductsService.groupProductsByProvider(xmlProducts, invoices);
          } catch (error) {
            console.error('Error cargando productos XML para orden', order.folio, error);
          }
        }

        // Asegurarnos de que fecha sea un objeto Date válido
        const fechaDate = order.fecha ? new Date(order.fecha) : new Date();

        // Calcular totales
        const subtotal = productosPorProveedor.reduce((sum, p) => sum + p.montoTotal, 0) || order.presupuesto || 0;
        ...
        console.log('Orden mapeada Pre-OC:', {
          folio: mappedOrder.folio,
          vehicle: mappedOrder.vehicle,
          fecha: mappedOrder.fecha,
          presupuesto: mappedOrder.presupuesto,
          processedProductsCount: mappedOrder.processedProductsCount,
          productosPorProveedor: mappedOrder.productosPorProveedor.length
        });

        return mappedOrder;
      }));

      // Asignar el array de una sola vez
      this.pendingPreOCOrders = mappedOrders;
      this.pendingPreOCCount = this.pendingPreOCOrders.length;

      console.log('Total órdenes Pre-OC cargadas:', this.pendingPreOCCount);
      console.log('Array pendingPreOCOrders:', this.pendingPreOCOrders);
      ...
      this.isLoadingPreOCOrders = false;
    }
  }
  ...
  // Funciones helper para cálculos financieros en la lista Pre-OC
  getOrderSubtotal(order: Order): number {
    // Si ya tiene productosPorProveedor cargados, usar esos datos
    if (order.productosPorProveedor && order.productosPorProveedor.length > 0) {
      return order.productosPorProveedor.reduce((total, proveedor) => total + proveedor.montoTotal, 0);
    }
    // Si no, usar el presupuesto almacenado
    return order.presupuesto || 0;
  }

  getOrderIVA(order: Order): number {
    return this.getOrderSubtotal(order) * 0.16;
  }

  getOrderTotalWithIVA(order: Order): number {
    return this.getOrderSubtotal(order) + this.getOrderIVA(order);
  }

  getOrderMargin(order: Order): number {
    // Calcular margen de utilidad basado en productos procesados
    if (order.productosPorProveedor && order.productosPorProveedor.length > 0) {
      let totalProductos = 0;
      let sumaMargenes = 0;

      order.productosPorProveedor.forEach(proveedor => {
        proveedor.productos.forEach(producto => {
          // Si el producto ya tiene el margen calculado (como porcentaje), usarlo
          if (producto.margen !== undefined && producto.margen !== null && producto.margen > 0) {
            sumaMargenes += producto.margen;
            totalProductos++;
          } else {
            // Si no, calcularlo desde costo y precio de venta
            const costo = producto.precio || 0;
            const venta = (producto as any).precio_venta || producto.precioVenta || 0;

            if (costo > 0 && venta > 0) {
              const margenCalculado = ((venta - costo) / costo) * 100;
              sumaMargenes += margenCalculado;
              totalProductos++;
            }
          }
        });
      ...
      // Retornar el promedio de márgenes
      if (totalProductos === 0) return 0;
      return sumaMargenes / totalProductos;
    }
  ...
  async deleteOrder(order: Order) {
    ...
    try {
      ...
      const confirmacion = confirm(
        ...
        `Monto: $${formatCurrencyValue(order.presupuesto)}\n` +
        relatedDataMessage +
  ...
  calculateGroupSubtotal(productos: XmlProduct[]): number {
    return productos.reduce((sum, p) => sum + (p.subtotal || 0), 0);
  }

  calculateGroupIVA(productos: XmlProduct[]): number {
    return productos.reduce((sum, p) => sum + (p.iva || 0), 0);
  }

  getTotalProductsCount(): number {
    return this.xmlProducts.length;
  }

  getTotalSubtotal(): number {
    return this.xmlProducts.reduce((sum, p) => sum + (p.subtotal || 0), 0);
  }

  getTotalIVA(): number {
    return this.xmlProducts.reduce((sum, p) => sum + (p.iva || 0), 0);
  }

  getTotalWithIVA(): number {
    return this.getTotalSubtotal() + this.getTotalIVA();
  }

  toggleProveedor(index: number): void {
    if (this.productosPorProveedor[index]) {
      this.productosPorProveedor[index].expanded = !this.productosPorProveedor[index].expanded;
    }
  }
}
